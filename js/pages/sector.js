...
(function() {
	App.initSector = function(name) {
		if (typeof name === 'undefined') var name = App.academies[0].primary_cte_industry;
		...
		var updateDistributionChart = function(sector) {
			...
			for (var i = 0; i < App.spendingCategories.length; i++) {
				...
				App.academies.filter(function(d) { return d.primary_cte_industry === sector; })
					.forEach(function(d) {
	...
	var getAcademyData = function(a) {
		var events = App.academies.filter(function(d) { return d.primary_cte_industry === a; });
